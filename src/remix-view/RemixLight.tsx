...
export default function RemixLight() {
  ...
  const [solcVersion, setSolcVersionLocal] = useResource<string>('solcVersion', '');
  const [solcVersions] = useResource<string[]>('solcVersions', []);
  const [solcVersionLoading] = useResource<boolean>('solcVersionLoading', false);

  const handleVersionChange = (v: string) => {
    setSolcVersionLocal(v);
    // Send directly as solcVersion event so Compiler.setSolcVersion() handles it
  ...
  return <div className='flex flex-col space-y-4 h-[100vh]'>
    ...
    { useCompiler &&
      <>
        ...
        <Label label={solcVersionLoading ? 'Compiler Version (loading…)' : 'Compiler Version'}>
          <Dropdown
